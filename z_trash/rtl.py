...
This module consumes the Phase 8 query-analysis output and retrieves candidate
chunks using:
  1) semantic search over FAISS embeddings
  2) keyword search over BM25
  3) score fusion + light heuristic reranking
  4) optional graph expansion from chunks.json relationships
...
You can also request a richer output with scores and metadata.
...
python retrieval/rtl.py \
  --phase8 query_analysis/result.json \
...
class ChunkRecord:
    ...
    def retrieval_text(self) -> str:
        # Use the most retrieval-friendly text first.
...
def _tokenize(text: str) -> List[str]:
    # Keep alphanumerics and simple legal tokens like 375, 1(5), IPC.
...
class Phase9HybridRetriever:
    ...
    def _load_artifacts(self) -> None:
        ...
        if self.embedder is None:
            if SentenceTransformer is None:
                raise RuntimeError(
                    "sentence-transformers is not available and no custom embedder was supplied."
                )
            self.model = SentenceTransformer(self.embed_model_name)
            self.embedder = self._default_embedder
        self.reranker_model = CrossEncoder("BAAI/bge-reranker-large")

        # Basic alignment check between FAISS meta and chunks.
        if len(self.meta) and self.faiss_index is not None:
            if hasattr(self.faiss_index, "ntotal") and self.faiss_index.ntotal != len(self.meta):
                # Not fatal, but worth surfacing during debugging.
                pass
    ...
    def _default_embedder(self, text: str) -> List[float]:
        ...
        # BGE models work better with a search instruction.
        query = f"Represent this legal query for retrieving relevant statutory chunks: {text}"
        emb = self.model.encode([query], normalize_embeddings=True)
        if np is None:
            return emb[0].tolist()  # type: ignore[union-attr]
    ...
    def build_search_text(self, q8: Phase8Query) -> str:
        parts: List[str] = []
        parts.append(q8.query)
        parts.extend(q8.concepts)
        parts.extend(_flatten_targets(q8.targets))

        # Light domain expansion from intent.
        primary = q8.primary_intent
        intent_boost_terms = {
            "definition": ["meaning", "defined", "definition"],
            "explanation": ["explain", "explanation", "details"],
            "lookup": ["section", "subsection", "clause"],
            "legal_penalty": ["punishment", "penalty", "fine", "imprisonment"],
            "case_application": ["apply", "facts", "situation", "scenario"],
            "procedure": ["steps", "process", "procedure", "how to"],
            "consequence": ["what happens", "result", "liable"],
            "legal_exception": ["exception", "unless"],
            "legal_condition": ["condition", "when", "scope"],
            "comparison": ["difference", "compare", "vs"],
        }
        parts.extend(intent_boost_terms.get(primary, []))
    ...
    def retrieve(
        ...
        semantic_text = self.build_search_text(q8)
        ...
        merged = self._merge_hits(q8, faiss_hits, bm25_hits)
        ...
        reranked = self._semantic_rerank(q8, reranked[:10])
    ...
    def _semantic_rerank(self, q8, hits):
        if not hits:
            return hits
        ...
        for hit, score in zip(valid_hits, scores):
            hit.score += float(score) * 0.2  # ⚠️ small weight
    ...
    def _faiss_search(self, query_text: str, top_k: int) -> Dict[str, float]:
        ...
        if is_ip:
            ...
            norm_scores = _norm_list(raw_scores, inverse=False)
        else:
            # Lower L2 distance is better.
            raw_scores = distances
            norm_scores = _norm_list(raw_scores, inverse=True)
    ...
    def _bm25_search(self, query_tokens: List[str], top_k: int) -> Dict[str, float]:
        ...
        # Prefer a native BM25 object with get_scores().
        ...
        try:
            ...
            # Some implementations expect a raw string or different tokenization.
            try:
                scores = self.bm25.get_scores(" ".join(query_tokens))
            except Exception as exc:  # pragma: no cover
                raise RuntimeError(f"BM25 scoring failed: {exc}") from exc
    ...
    def _merge_hits(
        ...
        for cid in candidate_ids:
            ...
            merged[cid] = RetrievalHit(
                chunk_id=cid,
                score=base,
                source_scores={"faiss": fs, "bm25": bs},
                reasons=[],
            )
        return merged
    ...
    def _rerank(self, q8: Phase8Query, hits: Dict[str, RetrievalHit]) -> List[RetrievalHit]:
        ...
        for hit in hit_list:
            ...
            if overlap:
                bonus += min(0.18, 0.03 * overlap)
                reasons.append(f"term_overlap:{overlap}")

            # Target/section boost.
            ...
            if act_targets:
                ...
                if rec_act in act_targets or any(a in rec_act for a in act_targets):
                    bonus += 0.12
                    reasons.append("act_match")

            # Concepts and keywords.
            if target_values & keywords:
                bonus += 0.10
                reasons.append("concept_keyword_match")

            # Structural preference by intent.
            if q8.primary_intent == "definition" and rec.chunk_type in {"definition", "subsection", "content"}:
                bonus += 0.08
                reasons.append("definition_bias")
            elif q8.primary_intent == "legal_penalty" and rec.chunk_type in {"subsection", "clause", "content"}:
                bonus += 0.10
                reasons.append("penalty_bias")
            elif q8.primary_intent in {"legal_exception", "legal_condition"} and rec.chunk_type in {"explanation", "subsection", "clause"}:
                bonus += 0.08
                reasons.append("logic_bias")
            elif q8.primary_intent == "case_application" and rec.chunk_type in {"subsection", "clause", "content", "explanation"}:
                bonus += 0.06
                reasons.append("application_bias")

            # Graph-related boosts if the chunk has explanation/illustration relationships.
            if rec.references:
                bonus += 0.03
                reasons.append("has_refs")
            if rec.explains_node_id:
                bonus += 0.02
                reasons.append("is_explains")

            # If exact match was requested and chunk contains exact target string, boost more.
            if q8.requires_exact_match and q8.targets:
                ...
                if any(self._exact_text_match(text, et) or self._exact_text_match(" ".join(rec.keywords), et) for et in exact_targets):
                    bonus += 0.10
        ...
        for _ in range(self.graph_depth):
            ...
            for cid in frontier:
                ...
                for ref in rec.references:
                    ...
                    if tgt:
                        target_chunk = self._resolve_node_or_chunk_id(str(tgt))
                        if target_chunk:
                            related_ids.append(target_chunk)

                # Also bring explanation/illustration siblings when present.
                if rec.explains_node_id:
                ...
                for rid in related_ids:
                    if rid in seen:
                        continue
                    seen.add(rid)
                    target_rec = self.chunk_records.get(rid)
                    if target_rec is None:
                        continue
                    # Small graph boost; enough to keep context nearby but not overpower retrieval.
                    extra.append(
                        RetrievalHit(
                            chunk_id=rid,
                            score=0.15,
    ...
    def _resolve_node_or_chunk_id(self, maybe_id: str) -> Optional[str]:
        ...
        # Some references are stored by node_id under another chunk.
            ...
            payload.update(
                {
                    ...
                    "children_chunk_ids": rec.children_chunk_ids,
                }
            )
        return payload
...
def run_batch(
    phase8_results: List[Dict[str, Any]],
...
def main() -> None:
    ...
    parser.add_argument("--query", required=False, help="Single raw user query (will be analysed only if phase8 is absent)")
    ...
    parser.add_argument("--base-dir", default="/mnt/data", help="Directory containing faiss.index, bm25.pkl, chunks.json, faiss_meta.json")
    args = parser.parse_args()

    retriever = Phase9HybridRetriever.from_default_paths(
    ...
    if args.phase8:
        ...
        output = run_batch(
            ...
            graph_expand=not args.no_graph,
        )
    elif args.query:
        raise SystemExit(
            "This Phase 9 module expects Phase 8 output. Pass --phase8 result.json rather than --query, "
            "or add your Phase 8 analyser before calling retrieval."
        )
    else:
