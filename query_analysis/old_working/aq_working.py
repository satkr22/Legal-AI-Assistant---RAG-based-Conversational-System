...
def _normalize_subqueries(
    ...
    *,
    ...
    for subquery in subqueries:
        cleaned = _normalize_subquery_text(
            subquery,
            query=query,
            targets=targets,
            concepts=concepts,
...
def build_prompt(query: str, hints: Dict[str, Any], rule_output: Dict[str, Any]) -> str:
    schema = {
        ...
        "decomposition": {"needed": False, "sub_queries": []},
    }
...
6. If decomposition is needed, produce short standalone retrieval-friendly sub-queries.
...
def _normalize_output_shape(
    ...
    sub_queries = _coerce_str_list(decomposition.get("sub_queries", []))
    needed = bool(decomposition.get("needed", False))
    if not needed:
        sub_queries = []
    elif not sub_queries and rule_output.get("multi_intent"):
        if targets:
            sub_queries = list(targets[:4])
        elif concepts:
            sub_queries = list(concepts[:4])
