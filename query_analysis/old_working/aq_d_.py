...
def _derive_subquery_context(query: str, targets: Sequence[str], concepts: Sequence[str], subheading: Sequence[str]) -> str:
    if targets:
        return _join_brief_list(targets, max_items=2)
    if concepts:
        return _join_brief_list(concepts, max_items=3)
    if subheading:
        return _join_brief_list(subheading, max_items=3)
    return _normalize_space(query.rstrip("?."))


def _mentions_context(text: str, targets: Sequence[str], concepts: Sequence[str], subheading: Sequence[str]) -> bool:
    lowered = _normalize_space(text).lower()
    for item in list(targets) + list(concepts) + list(subheading):
...
def _normalize_subquery_text(
    ...
    *,
    ...
    context = _derive_subquery_context(query, targets, concepts, subheading)
    has_context = _mentions_context(sq, targets, concepts, subheading)
    ...
    *,
    ...
    for item in subqueries:
        if isinstance(item, dict):
            ...
            sq_subheading = _coerce_str_list(item.get("subheading", []))
        else:
...
class HintRetriever:
    ...
    def _keyword_overlap_hits(self, query: str, top_k: int) -> List[HintHit]:
        ...
        for record in self.records:
            terms = set()
            for phrase in list(record.keywords) + list(record.legal_concepts) + list(record.subheading):
...
def build_prompt(query: str, hints: Dict[str, Any], rule_output: Dict[str, Any]) -> str:
    ...
    schema = {
        ...
        "concepts": [],
        "targets": [],
        "subheading":hints.get("subheading"),
...
def _normalize_output_shape(
    ...
    subheading = _coerce_str_list(obj.get("subheading", []))
...
def _build_rule_only_output(
    ...
    subheading = _dedupe_keep_order(
        subheading
        for record in matched_records
        for subheading in record.subheading
        if subheading
    )
...
def _build_non_llm_fallback_output(
    ...
    subheading = _dedupe_keep_order(
        subheading
        for item in hint_items
        for subheading in _coerce_str_list(item.get("subheading", []))
    )
...
def _score_llm_consistency(result: Dict[str, Any], route: str) -> float:
    ...
    if isinstance(result.get("targets"), list):
        score += 0.15
    if isinstance(result.get("subheading"), list):
        score += 0.15
...
def _score_structure_validity(result: Dict[str, Any]) -> float:
    checks = [
        ...
        isinstance(result.get("subheading"), list),
