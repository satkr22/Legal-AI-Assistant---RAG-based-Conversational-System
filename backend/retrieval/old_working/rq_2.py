...
# Task 2: Domain-aware expansion for colloquial → legal term mapping.
# Maps everyday words to precise legal concepts for BM25 enrichment.
LEGAL_CONTEXT_EXPANSION: Dict[str, List[str]] = {
    "hit":    ["assault", "criminal force"],
    "beat":   ["assault", "criminal force"],
    "beaten": ["assault", "criminal force"],
    "hurt":   ["causing hurt"],
    "slap":   ["assault", "criminal force"],
    "punch":  ["assault", "criminal force"],
    "kick":   ["assault", "criminal force"],
    "attack": ["assault"],
    "threat": ["assault"],
    "threaten": ["assault"],
    "rob":    ["robbery", "theft"],
    "robbed": ["robbery", "theft"],
    "steal":  ["theft"],
    "stole":  ["theft"],
    "kill":   ["culpable homicide", "murder"],
    "killed": ["culpable homicide", "murder"],
...
# Task 2: Generic BM25 noise words — terms that add no legal discriminative signal.
BM25_GENERIC_NOISE = {
    "punishment", "punish", "punished", "law", "case", "person", "persons",
    "someone", "something", "general", "public", "legal", "crime", "criminal",
    "offense", "offence", "act", "section", "provision",
}

# Task 4: Terms that flag off-topic legal domains when no assault/hurt/force intent is present.
IRRELEVANT_DOMAIN_TERMS = [
    "public servant", "hospital", "judicial proceeding",
    "election", "contempt of court", "defamation",
]

# Task 4: Query-side signals indicating the query IS about force/assault.
ASSAULT_QUERY_SIGNALS = ["assault", "hurt", "force", "hit", "beat", "punch", "kick", "attack"]
...
class Phase9HybridRetriever:
    ...
    def _bm25_terms_for_query(self, q8: Phase8Query) -> List[str]:
        """Task 2: Build BM25 query terms with domain-aware expansion and noise filtering.

        Improvements over the original:
        - Strong legal concepts always lead the term list (highest BM25 discriminative value).
        - Generic noise words (punishment, law, case, person …) are stripped unless they
          appear alongside a substantive concept.
        - LEGAL_CONTEXT_EXPANSION maps colloquial tokens (hit, beat, hurt) → precise legal
          terms (assault, criminal force, causing hurt) so BM25 matches statutory language.
        - Total terms capped at 10 to keep the BM25 query focused.
        """
        max_terms = 10
        terms: List[str] = []

        # 1. Target values first (section numbers, named values — high-precision signals).
        terms.extend(_flatten_targets(q8.targets))

        # 2. Strong legal concepts — primary discriminative signal.
        #    Use a higher cap (6) so important multi-word concepts are not dropped.
        terms.extend(self._clean_concepts_for_bm25(q8, max_terms=6))

        # 3. Tokenise the raw query and apply expansions + noise filtering.
        tokens = _tokenize(q8.query)
        expanded: List[str] = []
        for token in tokens:
            if token in BM25_STOPWORDS:
                continue
            # Strip generic noise words that span too many unrelated legal sections.
            if token in BM25_GENERIC_NOISE:
                continue
            # Keep the raw token.
            expanded.append(token)
            # Apply standard morphological expansions (injure → hurt, steal → theft …).
            expanded.extend(LEGAL_TOKEN_EXPANSIONS.get(token, []))
            # Apply domain-aware colloquial → legal expansions (hit → assault …).
    ...
    def build_faiss_text(self, q8: Phase8Query) -> str:
        """Task 1: Build FAISS query text with legal concept grounding.

        Injecting top concepts (e.g. 'assault', 'criminal force') alongside the
        original query steers the embedding model away from generic terms like
        'punishment' toward the precise legal domain of the question.
        """
        parts = [q8.query]

        # Inject top 2 non-generic legal concepts for semantic grounding.
        # This ensures the embedding captures "assault / criminal force" rather than
        # the surface-level word "punishment" which spans many unrelated sections.
        if q8.concepts:
            good_concepts = [
                c for c in q8.concepts
                if _normalize_phrase(c) not in GENERIC_QUERY_CONCEPTS
            ]
            parts.extend(good_concepts[:2])

        # Include target values (section numbers, named entities, etc.)
    ...
    def _retrieve_for_variant(self, variant: QueryVariant) -> Dict[str, RetrievalHit]:
        ...
        # Normalise query concepts once (lowercase) for fast membership testing.
        query_concepts_lower = {_safe_lower(c) for c in (q8.concepts or [])}

        # Determine whether this query is about assault/force — used for domain penalty.
        query_lower = q8.query.lower()
        query_is_assault_related = any(sig in query_lower for sig in ASSAULT_QUERY_SIGNALS)
        ...
        for chunk_id, hit in hits.items():
            ...
            # ------------------------------------------------------------------
            # Task 3: Boost chunks whose legal_concepts overlap with the query
            # concepts. This lifts "assault / criminal force" sections above
            # generic "punishment" ones.
            # ------------------------------------------------------------------
            if query_concepts_lower and any(
                c in query_concepts_lower for c in rec.legal_concepts
            ):
                score *= 1.2
                hit.reasons.append("concept_match_boost")

            # ------------------------------------------------------------------
            # Task 4: Penalise chunks from unrelated legal domains when the
            # query is clearly about a different area (e.g., assault vs hospital
            # regulations). Only applied when the query has no overlap with those
            # domains itself, preventing false penalisation.
            # ------------------------------------------------------------------
            rec_text_lower = rec.text.lower()
            if any(term in rec_text_lower for term in IRRELEVANT_DOMAIN_TERMS):
                if not query_is_assault_related:
                    score *= 0.6
                    hit.reasons.append("irrelevant_domain_penalty")

            # ------------------------------------------------------------------
            # Task 5: Prioritise substantive content chunks over illustrative
            # examples. Illustrations are useful companions but should not
            # outrank the primary statutory text.
            # ------------------------------------------------------------------
    ...
    def _rerank(self, query_text: str, hits: Dict[str, RetrievalHit], limit: Optional[int] = None, q8: Optional["Phase8Query"] = None) -> List[RetrievalHit]:
        ...
        # Task 7: Enrich the rerank query with top legal concepts so the
        # CrossEncoder sees "Someone hit me assault criminal force" instead of
        # just "Someone hit me".  This gives the model the same legal grounding
        # we provide to FAISS and BM25.
        if q8 is not None and q8.concepts:
            good_concepts = [
                c for c in q8.concepts
                if _normalize_phrase(c) not in GENERIC_QUERY_CONCEPTS
            ]
            if good_concepts:
                rerank_query = query_text + " " + " ".join(good_concepts[:2])
            else:
    ...
    def _merge_variant_hits(self, per_variant: Dict[str, List[RetrievalHit]]) -> Dict[str, RetrievalHit]:
        ...
        for hit in merged.values():
            ...
            if support >= 2:
                # Task 6: Chunks corroborated by multiple sub-queries are more
                # likely to be genuinely relevant. Apply a stronger multiplicative
                # boost (×1.15) instead of the previous small additive nudge.
...
python retrieval/rq_2.py --phase8 query_analysis/result_2_.json  --base-dir data/processed/artifacts2  --k 5  --output retrieval/output_2__.json  --no-graph
