...
def _as_list_local(value: Any) -> List[Any]:
...
class EvidenceHit:
    ...
    def from_dict(cls, data: Dict[str, Any], corpus_chunk: Optional[CorpusChunk]) -> "EvidenceHit":
        ...
        return cls(
            ...
            semrank=dict(data.get("semrank", {}) or {}),
    ...
    def semantic_score(self) -> float:
        """SemRank relevance signal, with retrieval source scores as fallback."""
        candidates: List[float] = []
        for key in ("pre_rerank_score", "score", "best_similarity", "retrieval_score"):
            try:
                candidates.append(float(self.semrank.get(key, 0.0) or 0.0))
            except Exception:
                pass
        for key, value in self.source_scores.items():
            if "semrank" not in str(key).lower():
                continue
            try:
                candidates.append(float(value or 0.0))
            except Exception:
                pass
        return _clamp(max(candidates, default=0.0))

    def semantic_matches(self, min_similarity: float = 0.62) -> List[Dict[str, Any]]:
        rows = self.semrank.get("matched_concepts")
        ...
        for row in rows:
            if not isinstance(row, dict):
                continue
            try:
                sim = float(row.get("similarity", row.get("weighted_score", 0.0)) or 0.0)
            except Exception:
                sim = 0.0
    ...
    def semantic_match_terms(self, min_similarity: float = 0.62) -> List[str]:
        terms: List[str] = []
        for row in self.semantic_matches(min_similarity=min_similarity):
            query_concept = _normalize_space(row.get("query_concept"))
            chunk_concept = _normalize_space(row.get("chunk_concept"))
            if query_concept and chunk_concept:
                terms.append(f"{query_concept} ~ {chunk_concept}")
            elif query_concept:
                terms.append(query_concept)
            elif chunk_concept:
                terms.append(chunk_concept)
...
class StructuredLegalReasoner:
    ...
    def _identify_base_concept(
        ...
        """
        The base concept is the most frequent SemRank query concept seen in
        retrieved evidence. Exact concept overlap is not used for selection.
        """
        counter: Counter = Counter()
        for hit in hits:
            for row in hit.semantic_matches():
                query_concept = _safe_lower(row.get("query_concept"))
                if query_concept:
                    counter[query_concept] += 1
    ...
    def _condition_match_score(
        ...
        """
        Compute a condition-match score (0.0-1.0) using SemRank evidence.

        Exact concept overlap and keyword Jaccard are intentionally not used
        here. SemRank already tells us whether the query's legal/factual
        concepts semantically align with the chunk's concepts.
        """
        semantic_score = hit.semantic_score()
        matches = hit.semantic_matches()
        if not matches:
            # SemRank disabled/unavailable fallback: keep the reasoner alive,
            # but do not recreate exact overlap or keyword-based selection.
            return round(_clamp(hit.score * 0.25, 0.0, 0.45), 4)

        q_profile = {_safe_lower(c) for c in query_concepts}
        matched_query_terms = {
            _safe_lower(row.get("query_concept"))
            for row in matches
            if _safe_lower(row.get("query_concept"))
        }
        profile_ratio = len(matched_query_terms & q_profile) / max(len(q_profile), 1)

        legal_signal = 0.0
        factual_signal = 0.0
        for row in matches:
            try:
                weighted = float(row.get("weighted_score", row.get("similarity", 0.0)) or 0.0)
            except Exception:
                weighted = 0.0
            source = _safe_lower(row.get("source"))
            query_term = _safe_lower(row.get("query_concept"))
            if source == "legal_concept":
                legal_signal = max(legal_signal, weighted)
            if query_term and query_term not in q_profile:
                factual_signal = max(factual_signal, weighted)
        ...
        has_positive_condition = any(m in hit_text_lower for m in [
            " if ", " when ", " where ", " at the time ", " in committing ",
            " in attempting ", " with intent ", " by putting ", " armed with ",
        ])
        ...
        if negation_in_section and matches:
            base -= 0.12
        elif has_positive_condition and matches:
            base += 0.08
    ...
    def reason(
        ...
        for section_token, section_hits in hits_by_section.items():
            ...
            semantic_matches = best_hit.semantic_match_terms()
        ...
        if selected_analysis:
            ...
            if candidates_for_section:
                selected_hit = max(
                    ...
                    key=lambda h: (1 if h.is_base else 0, h.semantic_score(), h.score),
                )
        ...
        if selected_analysis:
            ...
            if selected_analysis.semantic_matches:
                reason_parts.append(
                    f"Semantic matches: {', '.join(selected_analysis.semantic_matches[:5])}."
                )
            if overrides:
                score_section = (
...
class Phase11Reasoner:
    ...
    def _lookup_target_hit(
        self,
        target_summary: RequestedTargetSummary,
        hits: List[EvidenceHit],
    ) -> Optional[EvidenceHit]:
        """
        For direct section lookup, preserve the explicitly requested section.

        Lookup queries are not application questions. If the requested section
        exists and was retrieved, Phase 11 should explain that section instead
        of letting condition reasoning select a neighboring provision.
        """
        if target_summary.support_status != "exact_section_supported":
            return None
        target_tokens = [
            _canonical_reference_token(target)
            for target in target_summary.retrieved_matches
            if _canonical_reference_token(target)
        ]
        if not target_tokens:
            return None

        def lookup_key(hit: EvidenceHit) -> Tuple[int, int, int, float, float, int]:
            hit_citation = _canonical_reference_token(hit.citation)
            corpus_citation = _canonical_reference_token(hit.corpus_chunk.citation_text if hit.corpus_chunk else "")
            section_token = hit.section_token or ""
            exact_citation = any(token in {hit_citation, corpus_citation} for token in target_tokens)
            base_section = any(section_token == token.split("(")[0] for token in target_tokens)
            return (
                1 if exact_citation else 0,
                1 if base_section else 0,
        ...
        matches = [
            hit
            for hit in hits
            if any(self._target_matches_hit(f"Section {token}", hit) for token in target_tokens)
        ]
        if not matches:
            return None
        return max(matches, key=lookup_key)

    def _lock_reasoning_to_lookup_target(
        self,
        sr: StructuredReasoningResult,
        locked_hit: Optional[EvidenceHit],
    ) -> StructuredReasoningResult:
        if not locked_hit:
            return sr
        section_label = f"Section {locked_hit.section_token}" if locked_hit.section_token else locked_hit.citation
        if not section_label:
            return sr
        sr.selected_section = section_label
        sr.selected_hit = locked_hit
        sr.overrides_score_selection = False
        sr.uncertainty = _UNC_LOW
        sr.uncertainty_reason = "Explicit lookup target was retrieved and preserved as the answer anchor."
        sr.selection_reason = (
            f"Selected {section_label} because the user requested that exact section and it was retrieved. "
            "Structured condition comparisons are diagnostic only for direct lookup queries."
        )
        return sr
    ...
    def _section_candidates(
        ...
        for token, section_hits in grouped.items():
            section_hits_sorted = sorted(section_hits, key=lambda h: (-h.score, -h.semantic_score(), h.rank))
            ...
            avg_semantic = sum(h.semantic_score() for h in section_hits) / max(len(section_hits), 1)

            score = best.score * 0.55 + avg_semantic * 0.30
    ...
    def _preferred_hit(
        ...
        if not candidates:
            return max(hits, key=lambda h: (h.score, h.semantic_score()))
    ...
    def _related_hits(
        ...
        same_section = sorted(same_section, key=lambda h: (-h.score, -h.semantic_score(), h.rank))
        ...
        other_hits = sorted(other_hits, key=lambda h: (-h.semantic_score(), -h.score, h.rank))
    ...
    def _prompt_evidence_hits(
        ...
        """Select the compact evidence set that is actually sent to the LLM."""
        if not hits:
            return []

        limit = self.top_support_hits
        selected: List[EvidenceHit] = []
        seen_chunk_ids: Set[str] = set()
        section_counts: Counter[str] = Counter()
        target_tokens = {
            token.split("(")[0]
            for token in (_canonical_reference_token(target) for target in target_summary.present_in_corpus)
            if token
        }
        max_per_section = 3 if intent_family == "lookup" and target_tokens else 2

        def section_key(hit: EvidenceHit) -> str:
            return hit.section_token or hit.chunk_id

        def add(hit: Optional[EvidenceHit], allow_section_overflow: bool = False) -> bool:
            if not hit or hit.chunk_id in seen_chunk_ids or len(selected) >= limit:
                return False
            sec = section_key(hit)
            if not allow_section_overflow and section_counts[sec] >= max_per_section:
                return False
            selected.append(hit)
        ...
        def quality_key(hit: EvidenceHit) -> Tuple[int, int, int, int, float, float, float, int]:
            sec_type = _safe_lower(hit.corpus_chunk.section_type if hit.corpus_chunk else "")
            return (
                1 if hit.section_token in target_tokens else 0,
                1 if hit.is_base else 0,
                SECTION_TYPE_PRIORITY.get(sec_type, 0),
                CHUNK_TYPE_PRIORITY.get(hit.chunk_type, 0),
                hit.semantic_score(),
        ...
        # Always preserve the strongest anchor chosen by the section-aware scorer.
        add(primary_hit, allow_section_overflow=True)

        # If the user asked for a concrete section, protect direct evidence for it.
        target_hits = sorted(
            [hit for hit in hits if hit.section_token in target_tokens],
            key=quality_key,
            reverse=True,
        )
        ...
        # Add nearby evidence from the same selected section before diversifying.
        if primary_hit:
            same_section = sorted(
                [
                    hit
                    for hit in hits
                    if hit.section_token == primary_hit.section_token and hit.chunk_id != primary_hit.chunk_id
                ],
                key=quality_key,
                reverse=True,
            )
            for hit in same_section:
                add(hit)

        # Fill remaining slots by global rerank order so strong retrieval evidence is not lost.
        for hit in sorted(hits, key=lambda h: (h.rank or 10**9)):
            add(hit)
    ...
    def _section_distribution(self, hits: List[EvidenceHit]) -> List[Dict[str, Any]]:
        ...
        ranked = sorted(
            grouped.items(),
            key=lambda item: (
                max((h.semantic_score() for h in item[1]), default=0.0),
                max((h.score for h in item[1]), default=0.0),
                len(item[1]),
            ),
            reverse=True,
        )
            ...
            out.append(
                {
                    ...
                    "semantic_relevance": round(max((hit.semantic_score() for hit in section_hits), default=0.0), 4),
    ...
    def _confidence_score(
        ...
        score += min(primary_hit.semantic_score(), 0.20)
    ...
    def _completion_state(
        ...
        if len(hits) == 1 and primary_hit.semantic_score() < 0.35:
            return "partial"
        return "complete"
    ...
    def _evidence_rows(self, hits: Sequence[EvidenceHit]) -> List[Dict[str, Any]]:
    ...
    def _allowed_grounding(self, hits: Sequence[EvidenceHit]) -> Dict[str, Any]:
        ...
        for hit in hits:
            ...
            if hit.section_token:
                section_tokens.append(hit.section_token)

        citation_tokens = {
            token
            for token in (_canonical_reference_token(citation) for citation in citations)
            if token
        }
        citation_bases = {token.split("(")[0] for token in citation_tokens}
        section_tokens = _dedupe_keep_order(section_tokens)
        citation_bases.update(section_tokens)

        return {
            "chunk_ids": chunk_ids,
            "citations": _dedupe_keep_order(citations),
            "citation_keys": {_safe_lower(citation) for citation in citations if citation},
            "citation_tokens": citation_tokens,
            "section_tokens": set(section_tokens),
            "section_bases": citation_bases,
        }

    def _citation_is_allowed(self, citation: str, allowed: Dict[str, Any]) -> bool:
        text = _normalize_space(citation)
        if not text:
            return False
        if _safe_lower(text) in allowed["citation_keys"]:
            return True
        token = _canonical_reference_token(text)
        if not token:
            return False
        return token in allowed["citation_tokens"] or token.split("(")[0] in allowed["section_bases"]

    def _selected_section_from_chunks(self, chunk_ids: Sequence[str], hits: Sequence[EvidenceHit]) -> Optional[str]:
        hit_by_id = {hit.chunk_id: hit for hit in hits}
        sections: List[str] = []
        for chunk_id in chunk_ids:
            hit = hit_by_id.get(_normalize_space(chunk_id))
            if hit and hit.section_token:
                sections.append(hit.section_token)
        if not sections:
            for hit in hits:
                if hit.section_token:
                    sections.append(hit.section_token)
    ...
    def _matched_sections_from_hits(self, hits: Sequence[EvidenceHit]) -> List[str]:
        return _dedupe_keep_order(
            [f"Section {hit.section_token}" for hit in hits if hit.section_token]
        )

    def _fallback_answer(self, query: str, hits: Sequence[EvidenceHit]) -> Dict[str, Any]:
        if not hits:
            text = "The current retrieval results do not provide enough evidence to compose a grounded answer."
            return {
                "summary_answer": text,
                ...
                "support": [],
                "reasoning_trace": [],
                "caution_notes": ["No retrieved chunks were available for Phase 11."],
            }

        selected = list(hits[: self.top_support_hits])
        citations = _dedupe_keep_order(
            [
                hit.corpus_chunk.citation_text if hit.corpus_chunk and hit.corpus_chunk.citation_text else hit.citation
                for hit in selected
                if hit.citation or (hit.corpus_chunk and hit.corpus_chunk.citation_text)
            ]
        )
        primary = selected[0]
        primary_section = f"Section {primary.section_token}" if primary.section_token else primary.citation or "the top retrieved chunk"
        title = primary.corpus_chunk.section_title if primary.corpus_chunk else ""
        title_part = f" ({title})" if title else ""
        summary = f"The retrieved evidence points to {primary_section}{title_part} as the strongest grounded source for this query."
        detail_parts = []
        for hit in selected:
            section = f"Section {hit.section_token}" if hit.section_token else hit.citation or hit.chunk_id
            citation = hit.corpus_chunk.citation_text if hit.corpus_chunk and hit.corpus_chunk.citation_text else hit.citation
            detail = _compact_text(hit.text, max_sentences=2, max_chars=360)
            if detail:
                detail_parts.append(f"{section}: {detail} [{citation}]")
        detailed = " ".join(detail_parts) or summary
        final = f"{summary} {detailed}".strip()
        return {
            "summary_answer": summary,
            "detailed_answer": detailed,
            "final_answer": final,
            "citations": citations,
            "selected_chunk_ids": [hit.chunk_id for hit in selected if hit.chunk_id],
            "support": [
                {
                    "point": "Deterministic fallback from retrieved evidence.",
                    "chunk_id": hit.chunk_id,
                    "citation": hit.corpus_chunk.citation_text if hit.corpus_chunk and hit.corpus_chunk.citation_text else hit.citation,
                }
                for hit in selected
            ...
            "reasoning_trace": [
                "Used the top globally reranked chunks only.",
                f"Primary retrieved section: {primary_section}.",
            ],
            "caution_notes": ["Deterministic fallback was used."],
        }

    def _strict_llm_system_prompt(self) -> str:
        return (
            "You are Phase 11 of a legal RAG pipeline. Answer only from the evidence chunks provided. "
            "Do not use outside law, outside facts, or unstated assumptions. Do not invent citations, sections, "
            "punishments, offences, ingredients, or outcomes. Every legal conclusion must be supported by a provided "
            "chunk_id and citation. If the evidence is insufficient, say so. Return only JSON with keys: "
            "summary_answer, detailed_answer, final_answer, citations, selected_chunk_ids, support, reasoning_trace, "
            "caution_notes. citations must be copied from evidence citation values. selected_chunk_ids must be copied "
            "from evidence chunk_id values. support must be a list of objects with point, chunk_id, citation."
        )

    def _strict_llm_user_prompt(
        self,
        query: str,
        phase8: Dict[str, Any],
        ...
        payload = {
            ...
            "intent": dict(phase8.get("intent", {}) or {}),
            "instructions": [
                "Use only the evidence array.",
                "Cite each legal conclusion using support objects.",
                "Use exact chunk_id and citation strings from evidence.",
                "Mention only sections that appear in evidence.",
                "Return concise summary_answer and fuller detailed_answer.",
            ],
            "evidence": evidence_rows,
        }
        if repair_issues:
            payload["repair_required"] = True
            payload["validation_errors_to_fix"] = repair_issues
    ...
    def _generate_evidence_answer(
        ...
        try:
            response = self.llm_client.generate_json(
                ...
                system_prompt=self._strict_llm_system_prompt(),
                user_prompt=self._strict_llm_user_prompt(
                    query=query,
                    phase8=phase8,
    ...
    def _coerce_answer_payload(self, raw: Dict[str, Any]) -> Dict[str, Any]:
        summary = _normalize_space(raw.get("summary_answer"))
        detailed = _normalize_space(raw.get("detailed_answer"))
        final = _normalize_space(raw.get("final_answer")) or detailed or summary
        citations = _dedupe_keep_order([_normalize_space(x) for x in _as_list_local(raw.get("citations"))])
        selected_chunk_ids = _dedupe_keep_order([_normalize_space(x) for x in _as_list_local(raw.get("selected_chunk_ids"))])
        support_rows: List[Dict[str, str]] = []
        for item in _as_list_local(raw.get("support")):
            if not isinstance(item, dict):
                continue
            support_rows.append(
                {
                    "point": _normalize_space(item.get("point")),
                    "chunk_id": _normalize_space(item.get("chunk_id")),
                    "citation": _normalize_space(item.get("citation")),
                }
            )
        return {
            "summary_answer": summary or final,
            "detailed_answer": detailed or final or summary,
            "final_answer": final or detailed or summary,
            "citations": citations,
            "selected_chunk_ids": selected_chunk_ids,
            "support": support_rows,
            "reasoning_trace": _coerce_str_list(raw.get("reasoning_trace")),
            "caution_notes": _coerce_str_list(raw.get("caution_notes")),
        }
    ...
    def _validate_grounded_answer(
        ...
        allowed = self._allowed_grounding(hits)
        issues: List[str] = []

        if not _normalize_space(answer.get("final_answer")):
            issues.append("final_answer is empty")
        if not _normalize_space(answer.get("summary_answer")):
            issues.append("summary_answer is empty")
        if not _normalize_space(answer.get("detailed_answer")):
            issues.append("detailed_answer is empty")

        selected_chunk_ids = [
            _normalize_space(chunk_id)
            for chunk_id in answer.get("selected_chunk_ids", [])
            if _normalize_space(chunk_id)
        ]
        if hits and not selected_chunk_ids:
            issues.append("selected_chunk_ids is empty")
        for chunk_id in selected_chunk_ids:
            if chunk_id not in allowed["chunk_ids"]:
                issues.append(f"Unsupported selected_chunk_id: {chunk_id}")

        citations = [
            _normalize_space(citation)
            for citation in answer.get("citations", [])
            if _normalize_space(citation)
        ]
        if hits and not citations:
            issues.append("citations is empty")
        for citation in citations:
            if not self._citation_is_allowed(citation, allowed):
                issues.append(f"Unsupported citation: {citation}")

        texts = [
            _normalize_space(answer.get("summary_answer")),
            _normalize_space(answer.get("detailed_answer")),
            _normalize_space(answer.get("final_answer")),
        ]
        for mention in _extract_section_references(" ".join(texts)):
            token = _canonical_reference_token(mention)
            if token and token.split("(")[0] not in allowed["section_bases"]:
                issues.append(f"Unsupported section reference in answer: {mention}")

        support = answer.get("support", [])
        if require_support and not support:
            issues.append("support metadata is missing")
        for item in support:
            if not isinstance(item, dict):
                issues.append("support item is not an object")
                continue
            chunk_id = _normalize_space(item.get("chunk_id"))
            citation = _normalize_space(item.get("citation"))
            if not chunk_id or chunk_id not in allowed["chunk_ids"]:
                issues.append(f"Unsupported support chunk_id: {chunk_id or '<empty>'}")
            if not citation or not self._citation_is_allowed(citation, allowed):
                issues.append(f"Unsupported support citation: {citation or '<empty>'}")

        return len(issues) == 0, _dedupe_keep_order(issues)

    def _lightweight_structured_reasoning(
        self,
        hits: Sequence[EvidenceHit],
        answer: Dict[str, Any],
        verified: bool,
        ...
        selected_section = self._selected_section_from_chunks(answer.get("selected_chunk_ids", []), hits)
        if fallback_used or not verified:
            uncertainty = _UNC_HIGH if not verified else _UNC_MEDIUM
        elif repaired:
            uncertainty = _UNC_MEDIUM
        else:
            uncertainty = _UNC_LOW

        if fallback_used:
            reason = "Deterministic fallback selected the top retrieved evidence after LLM grounding failed."
        elif repaired:
            reason = "LLM answer was repaired once and then passed retrieved-evidence guardrails."
        elif verified:
            reason = "LLM answer passed retrieved-evidence guardrails."
        else:
            reason = "LLM answer did not pass retrieved-evidence guardrails."

        sections_analyzed = []
        grouped: Dict[str, List[EvidenceHit]] = defaultdict(list)
        for hit in hits:
            grouped[hit.section_token or hit.chunk_id].append(hit)
        for token, section_hits in grouped.items():
            best = max(section_hits, key=lambda hit: (hit.score, hit.semantic_score()))
            sections_analyzed.append(
                {
                    "section": f"Section {token}" if best.section_token else token,
                    "role": "retrieved_evidence",
                    "condition_summary": _compact_text(best.text, max_sentences=1, max_chars=260),
                    "condition_match_score": round(best.score, 6),
                    "concept_overlap": [],
                    "semantic_matches": best.semantic_match_terms()[:5],
                    "raw_retrieval_score": round(best.score, 6),
                    "hit_count": len(section_hits),
        ...
        return {
            ...
            "overrides_score_selection": False,
            "uncertainty": uncertainty,
            "uncertainty_reason": "; ".join(issues[:3]) if issues else reason,
            "alternative_sections": [
                row["section"]
                for row in sections_analyzed
                if row.get("section") and row.get("section") != selected_section
            ][:3],
            ...
            evidence_rows.append(
                {
                    ...
                    "semantic_relevance": round(hit.semantic_score(), 4),
                    "semantic_matches": hit.semantic_match_terms()[:6],
                ...
                "sections_analyzed": [
                    {
                        ...
                        "semantic_matches": a.semantic_matches,
    ...
    def reason_one(self, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        prompt_hits = self._prompt_evidence_hits(hits, primary_hit, intent_family, target_summary)
        if prompt_hits:
            primary_hit = prompt_hits[0]
        related_hits = [hit for hit in prompt_hits if not primary_hit or hit.chunk_id != primary_hit.chunk_id]
        intent_alignment, intent_notes = self._intent_alignment(intent_family, prompt_hits, primary_hit)
        support_status = self._support_status_for_intent(prompt_hits, primary_hit, target_summary)
        base_warnings = self._warning_messages(prompt_hits, primary_hit, target_summary, intent_alignment, intent_family)
        evidence_rows = self._evidence_rows(prompt_hits)

        llm_attempt = self._generate_evidence_answer(query=query, phase8=phase8, evidence_rows=evidence_rows)
        answer_payload = self._coerce_answer_payload(llm_attempt.get("response", {}))
        if llm_attempt.get("used"):
            verified, verification_issues = self._validate_grounded_answer(
                answer_payload,
                hits=prompt_hits,
                require_support=True,
            )
        else:
            verified, verification_issues = False, []
        llm_verification_issues = list(verification_issues)

        repaired = False
        repair_attempt: Dict[str, Any] = {"used": False, "error": None}
        if llm_attempt.get("used") and not verified:
            repair_attempt = self._generate_evidence_answer(
                query=query,
                phase8=phase8,
                evidence_rows=evidence_rows,
                repair_issues=verification_issues,
            )
            repaired_payload = self._coerce_answer_payload(repair_attempt.get("response", {}))
            repaired_verified, repaired_issues = self._validate_grounded_answer(
                repaired_payload,
                hits=prompt_hits,
                require_support=bool(repair_attempt.get("used")),
            )
            if repaired_verified:
                ...
                verification_issues = _dedupe_keep_order(verification_issues + repaired_issues)
                llm_verification_issues = list(verification_issues)

        fallback_used = False
        if not verified:
            fallback_used = True
            answer_payload = self._fallback_answer(query, prompt_hits)
            fallback_verified, fallback_issues = self._validate_grounded_answer(
                answer_payload,
                hits=prompt_hits,
                require_support=False,
            )
            verified = bool(fallback_verified and prompt_hits)
            verification_issues = fallback_issues

        citations = _dedupe_keep_order(answer_payload.get("citations", []))
        selected_chunk_ids = _dedupe_keep_order(answer_payload.get("selected_chunk_ids", []))
        matched_sections = self._matched_sections_from_hits(prompt_hits)
        sr_output = self._lightweight_structured_reasoning(
            hits=prompt_hits,
        ...
        completeness = "complete" if verified and not fallback_used else ("partial" if hits and verified else "unsafe")
        grounding_strength = 1.0 if verified and not repaired and not fallback_used else (0.72 if verified else 0.25)
        confidence = round(_clamp(0.35 + (max((hit.score for hit in hits), default=0.0) * 0.35) + grounding_strength * 0.28), 4)
        warnings = _dedupe_keep_order(
            base_warnings
            + answer_payload.get("caution_notes", [])
            + (
                ["LLM answer failed grounding checks; deterministic fallback was used."]
                if fallback_used and hits and llm_attempt.get("used")
                else ["LLM answer was unavailable; deterministic fallback was used."]
                if fallback_used and hits
                else []
            )
            + ([f"Grounding issue: {issue}" for issue in verification_issues] if verification_issues else [])
        )
        evidence_distribution = self._section_distribution(prompt_hits)
        llm_used = bool(llm_attempt.get("used") and not fallback_used)
        llm_error = llm_attempt.get("error") or repair_attempt.get("error")
        ...
        phase11 = {
            ...
            "summary_answer": answer_payload.get("summary_answer", ""),
            "detailed_answer": answer_payload.get("detailed_answer", ""),
            "citations": citations,
            ...
            "validation": {
                ...
                "llm_answer_verified": bool(llm_used and verified),
                "llm_verification_issues": [] if llm_used and verified else llm_verification_issues,
                "completeness": completeness,
                "guardrail_grounding_strength": grounding_strength,
                "repaired": repaired,
                "fallback_used": fallback_used,
            },
            "evidence": {
                "primary_chunk_id": selected_chunk_ids[0] if selected_chunk_ids else (primary_hit.chunk_id if primary_hit else None),
                "primary_chunk_type": primary_hit.chunk_type if primary_hit else None,
                "primary_section": sr_output.get("selected_section"),
                "primary_section_title": primary_hit.corpus_chunk.section_title if primary_hit and primary_hit.corpus_chunk else "",
                "related_chunk_ids": selected_chunk_ids[1:],
                "top_sections": evidence_distribution,
                "llm_evidence": evidence_rows,
            },
            "llm_generation": {
                "enabled": llm_attempt.get("enabled", False),
                "used": llm_used,
                "model": llm_attempt.get("model") or repair_attempt.get("model"),
                "reasoning_trace": answer_payload.get("reasoning_trace", []),
                "caution_notes": answer_payload.get("caution_notes", []),
                "error": llm_error,
                "repaired": repaired,
                "fallback_used": fallback_used,
            },
