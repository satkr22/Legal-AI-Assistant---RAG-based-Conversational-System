...
# SemRank-first retrieval/scoring defaults. SemRank becomes the primary concept
# relevance guard; exact concept overlap remains a small diagnostic/bonus only.
SEMRANK_LEGAL_CONCEPT_WEIGHT = 1.0
SEMRANK_KEYWORD_WEIGHT = 0.75
SEMRANK_DIAGNOSTIC_LIMIT = 5
EXACT_CONCEPT_MINOR_BONUS = 0.03
SEMRANK_WEAK_MATCH_BOOST = 1.03
SEMRANK_NO_MATCH_PENALTY = 0.9
...
def _flatten_targets(targets: List[Any]) -> List[str]:
    vals: List[str] = []
    for t in targets or []:
        if isinstance(t, str):
            t_val = t.strip()
...
def _count_conditional_markers(text: str) -> int:
    """Count how many distinct conditional markers appear in *text*.

    Uses a sliding 3-word window so multi-word markers like "subject to" and
    "only if" are matched without splitting them across word boundaries.
    Each marker is counted at most once per text (presence, not frequency).
    """
    if not text:
        return 0
    ...
    for marker in CONDITIONAL_MARKERS:
        ...
        for i in range(len(words) - marker_len + 1):
            if words[i: i + marker_len] == marker_words:
                hit_count += 1
                break  # count each marker at most once
    return hit_count


def _conditional_marker_density(text: str) -> float:
    """Return the ratio of distinct conditional markers to total word count.

    A value ≥ CONDITIONAL_MARKER_DENSITY_THRESHOLD signals a highly
    qualified, specialized clause rather than a broad legal definition.
    """
    words = text.split()
    word_count = len(words)
    if word_count == 0:
        return 0.0
    return _count_conditional_markers(text) / word_count


def _is_general_clause(text: str) -> bool:
    """Return True if the chunk text looks like a broad, definitional clause.

    A clause is considered general when it is short (≤ GENERAL_CLAUSE_MAX_WORDS)
    AND its conditional marker density is below the threshold. Short, unconditional
    text is characteristic of core definitions and principal obligations — the
    provisions that should rank highest for most legal queries.

    Heuristic is fully domain-agnostic: no topic-specific keywords used.
    """
    words = text.split()
    if not words or len(words) > GENERAL_CLAUSE_MAX_WORDS:
        return False
    return _conditional_marker_density(text) < CONDITIONAL_MARKER_DENSITY_THRESHOLD


def _is_highly_conditional(text: str) -> bool:
    """Return True if the chunk is heavily loaded with conditional language.

    A high conditional marker density indicates a niche exception or specialized
    provision rather than a general rule — it should rank slightly lower when
    the query is seeking a core definition or broad explanation.

    Shares the same scanner as _is_general_clause via _conditional_marker_density.
    """
    if not text.split():
        return False
    return _conditional_marker_density(text) >= CONDITIONAL_MARKER_DENSITY_THRESHOLD
...
class Phase8Query:
    ...
    def with_inherited_concepts(self, parent_concepts: List[str]) -> "Phase8Query":
        """Return a copy of this query whose concept list is supplemented by
        *parent_concepts* for any concepts not already present.

        Used to ensure sub-queries that lack their own concepts still benefit
        from the parent query's domain grounding during FAISS enrichment and
        BM25 term building. Only concepts absent from self.concepts are added,
        and they are appended at the end so the sub-query's own concepts keep
        their priority.
        """
        if not parent_concepts:
            return self
        existing_lower = {_normalize_phrase(c) for c in self.concepts}
        additions = [
            c for c in parent_concepts
            if _normalize_phrase(c) not in existing_lower
        ]
        if not additions:
            return self
        # Build a minimal replacement without mutating the original dataclass.
        import dataclasses as _dc
        return _dc.replace(self, concepts=list(self.concepts) + additions)

    @property
    def enriched_query(self) -> str:
        """The FAISS query text that will be used for this query object.

        Exposed as a property so it can be logged / included in output
        without re-running the enrichment logic.
        """
        return enrich_query(self.query, self.concepts)
...
class RetrievalHit:
    ...
    semrank: Dict[str, Any] = field(default_factory=dict)
...
class Phase9HybridRetriever:
    def __init__(
        ...
        enable_semrank: bool = True,
        semrank_top_k: int = 20,
        semrank_weight: float = 0.18,
        semrank_strong_sim: float = 0.72,
        semrank_min_sim: float = 0.62,
        subquery_preserve_top_n: int = 2,
        subquery_preserve_min_score: float = 0.62,
        ...
        self.enable_semrank = enable_semrank
        self.semrank_top_k = max(0, int(semrank_top_k))
        self.semrank_weight = _clamp(float(semrank_weight), 0.0, 0.8)
        self.semrank_strong_sim = _clamp(float(semrank_strong_sim), 0.0, 1.0)
        self.semrank_min_sim = _clamp(float(semrank_min_sim), 0.0, 1.0)
        if self.semrank_strong_sim < self.semrank_min_sim:
            self.semrank_strong_sim = self.semrank_min_sim
        self.subquery_preserve_top_n = max(0, int(subquery_preserve_top_n))
        self.subquery_preserve_min_score = _clamp(float(subquery_preserve_min_score), 0.0, 1.0)
        self.max_chunks_per_section = max(1, int(max_chunks_per_section))
        self.semrank_max_query_concepts = 8
        self.semrank_entries: List[Dict[str, Any]] = []
        self.semrank_vectors: Any = None
        self.semrank_chunk_to_indices: Dict[str, List[int]] = defaultdict(list)
        self.semrank_query_cache: Dict[str, Tuple[List[str], Any]] = {}
    ...
    def _load_artifacts(self) -> None:
        ...
        self.semrank_entries = self._build_semrank_concept_entries() if self.enable_semrank else []
    ...
    def _clean_concepts_for_bm25(self, q8: Phase8Query, max_terms: int = 6) -> List[str]:
        """Return the top *max_terms* non-generic Phase 8 concepts for BM25.

        Default raised to 6 (was 4) to match actual usage — multi-word concepts
        such as "causing grievous hurt" consume one slot but carry high IDF value.
        """
        terms: List[str] = []
    ...
    def _build_query_variants(self, q8: Phase8Query) -> List[QueryVariant]:
        """Build the list of query variants: the original query plus any sub-queries.

        Sub-query concept inheritance
        -----------------------------
        Sub-queries produced by the decomposer often have sparse or empty concept
        lists because the decomposer focuses on splitting the query text, not on
        re-running concept extraction. A sub-query like "What is the punishment?"
        with no concepts of its own would get zero enrichment from enrich_query()
        and produce a poor FAISS embedding.

        Solution: each sub-query inherits the parent's concepts for any slots it
        does not fill itself, via Phase8Query.with_inherited_concepts(). The
        sub-query's own concepts remain first (higher priority); parent concepts
        are appended only to fill gaps.
        """
        variants: List[QueryVariant] = [
            QueryVariant(name="original", q8=q8, weight=1.0, source="original")
        ]
        ...
        for i, item in enumerate(q8.sub_queries or [], start=1):
            ...
            # Inherit parent concepts for concept-poor sub-queries.
            sub_q8 = sub_q8.with_inherited_concepts(q8.concepts)

            variants.append(
                QueryVariant(name=f"sub_query_{i}", q8=sub_q8, weight=0.9, source="decomposed")
            )

        return variants

    def _bm25_terms_for_query(self, q8: Phase8Query) -> List[str]:
        """Build a clean, focused BM25 term list — fully domain-agnostic.
    ...
    # -----------------------------
    # SemRank concept retrieval / scoring
    # -----------------------------

    def _is_good_semrank_phrase(self, text: str) -> bool:
        phrase = _normalize_phrase(text)
        if not phrase or phrase in GENERIC_QUERY_CONCEPTS:
            return False
        toks = phrase.split()
        if not toks or len(toks) > 8:
            return False
        if all(t in BM25_STOPWORDS or t in BM25_SURFACE_NOISE for t in toks):
            return False
        return True

    def _semrank_terms_for_chunk(self, rec: ChunkRecord) -> List[Tuple[str, float, str]]:
        terms: List[Tuple[str, float, str]] = []
        seen = set()
        for raw in rec.legal_concepts or []:
            phrase = _normalize_phrase(raw)
            if not self._is_good_semrank_phrase(phrase) or phrase in seen:
                continue
            seen.add(phrase)
            terms.append((phrase, SEMRANK_LEGAL_CONCEPT_WEIGHT, "legal_concept"))
        for raw in rec.keywords or []:
            phrase = _normalize_phrase(raw)
            if not self._is_good_semrank_phrase(phrase) or phrase in seen:
                continue
            seen.add(phrase)
            terms.append((phrase, SEMRANK_KEYWORD_WEIGHT, "keyword"))
        return terms

    def _build_semrank_concept_entries(self) -> List[Dict[str, Any]]:
        entries_by_key: Dict[Tuple[str, str], Dict[str, Any]] = {}
        for chunk_id, rec in self.chunk_records.items():
            if not rec.embedding_text:
                continue
            for phrase, weight, source in self._semrank_terms_for_chunk(rec):
                key = (phrase, chunk_id)
        ...
        entries.sort(key=lambda item: (item["phrase"], item["chunk_id"]))
        self.semrank_chunk_to_indices = defaultdict(list)
        for idx, entry in enumerate(entries):
            self.semrank_chunk_to_indices[str(entry["chunk_id"])].append(idx)
        return entries

    def _encode_semrank_phrases(self, phrases: Sequence[str]) -> Any:
        if not phrases or self.model is None or np is None:
            return None
        texts = [f"Represent this legal concept for retrieval: {phrase}" for phrase in phrases]
        try:
    ...
    def _ensure_semrank_index(self) -> bool:
        if not self.enable_semrank or self.model is None or np is None:
            return False
        if not self.semrank_entries:
            return False
        if self.semrank_vectors is not None:
            return True

        vectors = self._encode_semrank_phrases([str(entry["phrase"]) for entry in self.semrank_entries])
        if vectors is None or len(vectors) != len(self.semrank_entries):
            self.semrank_vectors = None
            return False
        self.semrank_vectors = vectors
        return True

    def _semrank_query_phrases(self, q8: Phase8Query) -> List[str]:
        phrases: List[str] = []
        for raw in q8.concepts or []:
            phrase = _normalize_phrase(raw)
            if self._is_good_semrank_phrase(phrase):
                phrases.append(phrase)

        if len(phrases) < self.semrank_max_query_concepts:
            for raw in self._bm25_terms_for_query(q8):
                phrase = _normalize_phrase(raw)
                if self._is_good_semrank_phrase(phrase):
                    phrases.append(phrase)

        if len(phrases) < self.semrank_max_query_concepts:
            fallback = _normalize_phrase(q8.query)
            if fallback:
                phrases.append(fallback)

        return _unique_keep_order(phrases)[: self.semrank_max_query_concepts]

    def _semrank_query_vectors(self, q8: Phase8Query) -> Tuple[List[str], Any]:
        phrases = self._semrank_query_phrases(q8)
        if not phrases:
            return [], None
        cache_key = json.dumps(phrases, sort_keys=True)
        cached = self.semrank_query_cache.get(cache_key)
        if cached is not None:
            return cached
        vectors = self._encode_semrank_phrases(phrases)
        if vectors is None:
            return phrases, None
        self.semrank_query_cache[cache_key] = (phrases, vectors)
        return phrases, vectors

    def _semrank_alignment_for_chunk(self, q8: Phase8Query, rec: ChunkRecord) -> Optional[Dict[str, Any]]:
        if not self._ensure_semrank_index():
            return None
        query_phrases, query_vectors = self._semrank_query_vectors(q8)
        indices = self.semrank_chunk_to_indices.get(rec.chunk_id, [])
        if not query_phrases or query_vectors is None or not indices:
            return None

        try:
            chunk_vectors = self.semrank_vectors[indices]
            sim_matrix = np.matmul(query_vectors, chunk_vectors.T)
        except Exception:
            return None
        ...
        for local_pos, idx in enumerate(indices):
            col = sim_matrix[:, local_pos]
            q_pos = int(col.argmax())
            sim = float(col[q_pos])
            entry = self.semrank_entries[idx]
            weighted = _clamp(sim * float(entry.get("weight", 1.0)))
            if weighted > best_weighted:
                best_weighted = weighted
                best_similarity = sim
                best_entry = entry
            if sim >= self.semrank_min_sim:
                matches.append(
                    {
                        "query_concept": query_phrases[q_pos],
                        "chunk_concept": entry["phrase"],
                        "source": entry.get("source", "concept"),
        ...
        return {
            "enabled": True,
            "query_concepts": query_phrases,
            "score": round(best_weighted, 4),
            "best_similarity": round(best_similarity, 4),
            "best_concept": best_entry["phrase"] if best_entry else None,
            "best_concept_source": best_entry.get("source", "concept") if best_entry else None,
            "matched_concepts": matches[:SEMRANK_DIAGNOSTIC_LIMIT],
        }

    def _semrank_search(self, q8: Phase8Query, top_k: int) -> List[Tuple[str, float, Dict[str, Any]]]:
        if top_k <= 0 or not self._ensure_semrank_index():
            return []
        query_phrases, query_vectors = self._semrank_query_vectors(q8)
        if not query_phrases or query_vectors is None:
            return []

        try:
            sims = np.matmul(query_vectors, self.semrank_vectors.T).max(axis=0)
        except Exception:
            return []

        concept_order = np.argsort(-sims)[: max(top_k * 8, top_k)]
        chunk_scores: Dict[str, float] = {}
        chunk_matches: Dict[str, List[Dict[str, Any]]] = defaultdict(list)

        for idx in concept_order:
            sim = float(sims[int(idx)])
            if sim < self.semrank_min_sim:
                continue
            entry = self.semrank_entries[int(idx)]
            chunk_id = str(entry["chunk_id"])
            weighted = _clamp(sim * float(entry.get("weight", 1.0)))
            chunk_scores[chunk_id] = max(chunk_scores.get(chunk_id, 0.0), weighted)
            chunk_matches[chunk_id].append(
                {
                    "concept": entry["phrase"],
                    "source": entry.get("source", "concept"),
                    "similarity": round(sim, 4),
                    "weighted_score": round(weighted, 4),
                }
        ...
        for chunk_id, score in ranked:
            matches = sorted(
                chunk_matches.get(chunk_id, []),
                key=lambda item: item["weighted_score"],
                reverse=True,
            )[:SEMRANK_DIAGNOSTIC_LIMIT]
            out.append(
                (
                    chunk_id,
                    _clamp(score),
                    {
                        "enabled": True,
                        "source": "retrieval",
                        "retrieval_score": round(_clamp(score), 4),
                        "query_concepts": query_phrases,
    ...
    def _apply_semrank_pre_rerank(self, hits: Dict[str, RetrievalHit], q8: Phase8Query) -> Dict[str, RetrievalHit]:
        if not hits or self.semrank_weight <= 0.0 or not self._ensure_semrank_index():
            return hits
        ...
        for chunk_id, hit in hits.items():
            ...
            if not rec:
                continue
            diag = self._semrank_alignment_for_chunk(q8, rec)
            if not diag or float(diag.get("best_similarity", 0.0) or 0.0) < self.semrank_min_sim:
                continue

            score = float(diag.get("score", 0.0) or 0.0)
            old_score = hit.score
            hit.score = _clamp((1.0 - self.semrank_weight) * hit.score + self.semrank_weight * score)
            hit.source_scores["semrank_pre_rerank"] = max(hit.source_scores.get("semrank_pre_rerank", 0.0), score)
            hit.reasons.append("semrank_pre_rerank")
            hit.reasons.append("semrank_concept_match")
            current = dict(hit.semrank or {})
            current.update(diag)
            current.update(
                {
                    "pre_rerank_score": round(score, 4),
                    "score_before_pre_rerank": round(old_score, 6),
                    "score_after_pre_rerank": round(hit.score, 6),
                }
            )
            hit.semrank = current
    ...
    def _retrieve_for_variant(self, variant: QueryVariant) -> Dict[str, RetrievalHit]:
        ...
        semrank_hits = self._semrank_search(q8, self.semrank_top_k) if self.enable_semrank else []
        ...
        def add_hit(
            ...
            score: float,
            reason: str,
            semrank_diag: Optional[Dict[str, Any]] = None,
        ) -> None:
            if not chunk_id:
                return
            hit = hits.get(chunk_id)
        ...
        semrank_raw = [s for _, s, _ in semrank_hits]
        semrank_norm = _normalize_scores_to_unit(semrank_raw)
        for rank, ((chunk_id, raw_score, diag), norm_score) in enumerate(zip(semrank_hits, semrank_norm)):
            semrank_score = max(_rank_scores(len(semrank_hits))[rank], norm_score, _clamp(raw_score))
            add_hit(
                chunk_id,
                "semrank",
                semrank_score * variant.weight,
                f"semrank_retrieval:{variant.name}",
                semrank_diag=diag,
            )
        ...
        for chunk_id, hit in hits.items():
            ...
            semrank_diag = self._semrank_alignment_for_chunk(q8, rec) if self.enable_semrank else None
            semantic_score = float((semrank_diag or {}).get("score", 0.0) or 0.0)
            semantic_similarity = float((semrank_diag or {}).get("best_similarity", 0.0) or 0.0)

            if semrank_diag is not None:
                current = dict(hit.semrank or {})
                current.update(semrank_diag)
                hit.semrank = current

                if semantic_similarity >= self.semrank_strong_sim:
                    score *= 1.0 + self.semrank_weight * semantic_score
                    hit.reasons.append("semrank_strong_match")
                elif semantic_similarity >= self.semrank_min_sim:
                    score *= SEMRANK_WEAK_MATCH_BOOST
                    hit.reasons.append("semrank_weak_match")
                elif query_concepts:
                    score *= SEMRANK_NO_MATCH_PENALTY
                    hit.reasons.append("semrank_no_match_soft_penalty")

            else:
                # Safe fallback for disabled/unavailable SemRank: keep the old
                # exact concept behavior so existing deployments do not regress.
                if overlap > 0:
    ...
    def _merge_variant_hits(self, per_variant: Dict[str, List[RetrievalHit]]) -> Dict[str, RetrievalHit]:
        """Merge per-variant hit lists into a single dict keyed by chunk_id.

        For each chunk seen across variants:
        - source_scores: union, keyed by "{variant}:{source}".
        - query_scores:  max per variant name.
        - score:         max across all variants (boosted post-merge for multi-support).
        - reasons:       deduplicated union across all variants (no repeated tags).
        """
        ...
        for variant_name, hits in per_variant.items():
            for rank, hit in enumerate(hits, start=1):
                ...
                m.source_scores.update(
                    {f"{variant_name}:{k}": v for k, v in hit.source_scores.items()}
                )
                m.query_scores[variant_name] = max(
                    m.query_scores.get(variant_name, 0.0), hit.score
                )
                m.reasons.extend(hit.reasons)
                m.score = max(m.score, hit.score)
                if hit.semrank:
                    existing_score = max(
                        float(m.semrank.get("pre_rerank_score", 0.0) or 0.0),
                        float(m.semrank.get("retrieval_score", 0.0) or 0.0),
                        float(m.semrank.get("score", 0.0) or 0.0),
                    ) if m.semrank else -1.0
                    incoming_score = max(
                        float(hit.semrank.get("pre_rerank_score", 0.0) or 0.0),
                        float(hit.semrank.get("retrieval_score", 0.0) or 0.0),
                        float(hit.semrank.get("score", 0.0) or 0.0),
                    )
                    if incoming_score >= existing_score:
                        m.semrank = dict(hit.semrank)

                if (
                    variant_name != "original"
                    and rank <= self.subquery_preserve_top_n
                    and hit.score >= self.subquery_preserve_min_score
                ):
                    m.reasons.append(f"subquery_preserved:{variant_name}")

        for hit in merged.values():
            # Deduplicate reason tags accumulated across variants. The same tag
            # (e.g. "concept_overlap:2") appearing twice is noise, not signal.
            hit.reasons = list(_unique_keep_order(hit.reasons))
    ...
    def _graph_expand(self, hits: Dict[str, RetrievalHit], q8: Phase8Query) -> Dict[str, RetrievalHit]:
        """Expand the hit set by traversing structural links from top-scoring seeds.

        Scoring for graph-introduced chunks
        ------------------------------------
        Previously, graph-introduced chunks received a raw score derived purely
        from their seed's score, bypassing all concept/clause adjustments. This
        meant an irrelevant neighbor of a high-scoring seed could sneak into the
        final results unpenalised.

        Now each newly introduced chunk goes through the same domain-relevance
        and clause-specificity adjustments as directly retrieved chunks:
        - Concept overlap boost (proportional).
        - Zero-overlap penalty if the query has a concept fingerprint.
        - General vs. conditional clause heuristic.
        - Chunk-type multiplier.

        The graph score is still discounted relative to the seed (×0.82) to
        reflect the lower confidence of structurally derived retrieval.
        """
        ...
        # Build query concept fingerprint once (same as in _retrieve_for_variant).
        query_concepts: set = {
            _safe_lower(c)
            for c in (q8.concepts or [])
            if _normalize_phrase(c) not in GENERIC_QUERY_CONCEPTS
        ...
        seeds = [
            cid for cid, _ in sorted(
                ((cid, h.score) for cid, h in hits.items()),
                key=lambda x: x[1],
                reverse=True,
            )[: self.final_k]
        ]
        ...
        for seed in seeds:
            neighbors = self._collect_neighbors(
                seed, depth=self.graph_depth, max_extra=self.graph_max_extra
            )
            for nid in neighbors:
                if nid in expanded:
                    continue
                ...
                if semrank_diag is not None:
                    if semantic_similarity >= self.semrank_strong_sim:
                        score = _clamp(score * (1.0 + self.semrank_weight * semantic_score))
                        reasons.append("semrank_strong_match")
                    elif semantic_similarity >= self.semrank_min_sim:
                        score = _clamp(score * SEMRANK_WEAK_MATCH_BOOST)
                        reasons.append("semrank_weak_match")
                    elif query_concepts:
                        score = _clamp(score * SEMRANK_NO_MATCH_PENALTY)
                        reasons.append("semrank_no_match_soft_penalty")

                else:
                    if overlap > 0:
                        score = _clamp(score * (1.0 + CONCEPT_MATCH_MULTIPLIER * overlap))
                        reasons.append(f"concept_overlap:{overlap}")
                    elif query_concepts:
                        score = _clamp(score * ZERO_OVERLAP_PENALTY)
                        reasons.append("zero_concept_overlap_penalty")

                if _is_general_clause(rec.text):
                    score = _clamp(score * GENERAL_CLAUSE_BOOST)
                    reasons.append("general_clause_boost")
                elif _is_highly_conditional(rec.text):
                    score = _clamp(score * CONDITIONAL_CLAUSE_PENALTY)
                    reasons.append("conditional_clause_penalty")

                if rec.chunk_type == "content":
                    score = _clamp(score * CONTENT_CHUNK_BOOST)
                    reasons.append("content_type_boost")
                elif rec.chunk_type == "illustration":
                    score = _clamp(score * ILLUSTRATION_CHUNK_PENALTY)
                    reasons.append("illustration_type_penalty")
    ...
    def _is_preserved_hit(self, hit: RetrievalHit) -> bool:
        return any(str(reason).startswith("subquery_preserved:") for reason in hit.reasons)

    def _chunk_type_priority(self, hit: RetrievalHit) -> int:
        rec = self.chunk_records.get(hit.chunk_id)
        if not rec:
            return 0
        if rec.chunk_type in {"content", "subsection", "clause", "section"}:
            return 3
        if rec.chunk_type == "explanation":
            return 2
        if rec.chunk_type == "illustration":
            return 1
        return 0

    def _section_key_for_hit(self, hit: RetrievalHit) -> str:
        rec = self.chunk_records.get(hit.chunk_id)
        if rec and rec.section_number is not None:
            return f"section:{rec.section_number}"
        return f"chunk:{hit.chunk_id}"

    def _diversify_hits(self, hits: Sequence[RetrievalHit], limit: Optional[int] = None) -> List[RetrievalHit]:
        if limit is None:
            limit = self.final_k
        if limit <= 0:
            return []
        ...
        ordered = sorted(
            list(hits),
            key=lambda hit: (
                1 if self._is_preserved_hit(hit) else 0,
                self._chunk_type_priority(hit),
                hit.score,
            ),
            reverse=True,
        )
        ...
        for hit in ordered:
            if len(selected) >= limit:
                break
            section_key = self._section_key_for_hit(hit)
            if section_counts[section_key] >= self.max_chunks_per_section:
                continue
            selected.append(hit)
    ...
    def _format_results(
        ...
        q8: Optional["Phase8Query"] = None,
    ) -> List[Dict[str, Any]]:
        # Pre-compute the normalised query concept set once for the whole result list.
        query_concepts: set = (
            {_normalize_phrase(c) for c in q8.concepts if _normalize_phrase(c)}
            if q8 is not None
            else set()
        )
        ...
        for rank, hit in enumerate(hits, start=1):
            ...
            # Concept coverage metrics — domain-agnostic overlap observability.
            chunk_concepts_norm = [
                _normalize_phrase(c) for c in (rec.legal_concepts or [])
            ]
            matched_concepts: List[str] = []
            if query_concepts:
                matched_concepts = [
                    c for c in chunk_concepts_norm if c in query_concepts
                ]
            concept_overlap_count = len(matched_concepts)
            concept_coverage_ratio = (
                round(concept_overlap_count / total_query_concepts, 4)
            ...
            row = {
                ...
                "concept_coverage": {
                    "query_concepts_total": total_query_concepts,
                    "matched_count": concept_overlap_count,
                    "coverage_ratio": concept_coverage_ratio,
            ...
            if hit.semrank:
                row["semrank"] = hit.semrank
            results.append(row)
        return results

    def _process_single_query(self, variant: QueryVariant) -> List[RetrievalHit]:
        """Retrieve, attach companions, and per-variant rerank for one query variant.

        Uses the weight already set on *variant* by _build_query_variants(), so
        there is a single source of truth for how original vs. sub-query results
        are weighted — no hardcoded duplication here.
        """
        raw_hits = self._retrieve_for_variant(variant)
        raw_hits = self._apply_semrank_pre_rerank(raw_hits, variant.q8)
        raw_hits = self._attach_anchor_chunks(raw_hits)
        raw_hits = self._attach_explanatory_companions(raw_hits, variant.q8)
        return self._rerank(variant.q8.query, raw_hits, limit=self.final_k, q8=variant.q8)
    ...
    def retrieve_one(self, phase8_item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for variant in variants:
            per_variant_reranked[variant.name] = self._process_single_query(variant)

        merged = self._merge_variant_hits(per_variant_reranked)
        merged = self._graph_expand(merged, q8)
        merged = self._apply_semrank_pre_rerank(merged, q8)

        results_without_global_rerank = self._format_results(
            self._diversify_hits(list(merged.values()), limit=self.final_k),
            q8=q8,
        )

        global_candidate_pool = self._diversify_hits(
            list(merged.values()),
            limit=max(self.final_k * 4, 20),
        )
        global_hits = self._rerank(
            q8.query,
            {hit.chunk_id: hit for hit in global_candidate_pool},
            limit=max(self.final_k * 4, 20),
            q8=q8,
        )
        results_with_global_rerank = self._format_results(
            self._diversify_hits(global_hits, limit=self.final_k),
            q8=q8,
        )
        ...
        return {
            ...
            "retrieval": {
                ...
                "per_query_results": {
                    name: self._format_results(hits, q8=variant_by_name.get(name, variants[0]).q8)
                ...
                "settings": {
                    ...
                    "semrank_enabled": self.enable_semrank,
                    "semrank_top_k": self.semrank_top_k,
                    "semrank_weight": self.semrank_weight,
                    "semrank_strong_sim": self.semrank_strong_sim,
                    "semrank_min_sim": self.semrank_min_sim,
                    "subquery_preserve_top_n": self.subquery_preserve_top_n,
                    "subquery_preserve_min_score": self.subquery_preserve_min_score,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--no-semrank", action="store_true", help="Disable SemRank concept retrieval and semantic scoring")
    p.add_argument("--semrank-top-k", type=int, default=20, help="SemRank candidate count per variant")
    p.add_argument("--semrank-weight", type=float, default=0.18, help="SemRank scoring/rerank blend weight")
    p.add_argument("--semrank-min-sim", type=float, default=0.62, help="Minimum SemRank similarity for weak matches")
    p.add_argument("--semrank-strong-sim", type=float, default=0.72, help="Minimum SemRank similarity for strong matches")
    p.add_argument("--subquery-preserve-top-n", type=int, default=1, help="Top hits to preserve from each sub-query")
    p.add_argument("--subquery-preserve-min-score", type=float, default=0.62, help="Minimum score for preserving sub-query hits")
    p.add_argument("--max-chunks-per-section", type=int, default=2, help="Maximum final evidence chunks per section")
    return p
...
def main() -> None:
    ...
    retriever = Phase9HybridRetriever(
        ...
        enable_semrank=not args.no_semrank,
        semrank_top_k=args.semrank_top_k,
        semrank_weight=args.semrank_weight,
        semrank_strong_sim=args.semrank_strong_sim,
        semrank_min_sim=args.semrank_min_sim,
        subquery_preserve_top_n=args.subquery_preserve_top_n,
        subquery_preserve_min_score=args.subquery_preserve_min_score,
