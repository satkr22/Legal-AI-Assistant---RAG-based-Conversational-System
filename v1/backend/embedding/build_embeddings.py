...
def get_bm25_text(chunk):
    parts = [
        chunk.get("embedding_text"),
        chunk.get("semantic_summary"),
        " ".join(str(x) for x in (chunk.get("keywords") or []) if str(x).strip()),
        " ".join(str(x) for x in (chunk.get("legal_concepts") or []) if str(x).strip()),
    ]
...
def main():
    ...
    for chunk in chunks:
        ...
        texts.append(t)
        bm25_texts.append(get_bm25_text(chunk) or t)
    ...
    # 4. BM25 INDEX
    # -------------------------
    tokenized = [tokenize(t) for t in bm25_texts]
