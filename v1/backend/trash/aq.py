...
This module turns a natural-language legal question into structured retrieval
signals:
- intent
- targets
- concepts
...
It is designed to work with a chunks.json artifact that contains section /
subsection / clause / explanation / illustration chunks. The chunk file is used
as grounding material for the LLM prompt, not as a replacement for retrieval.
...
def _is_good_concept_phrase(text: str) -> bool:
    ...
    if len(tokens) > 5:
        return False
    # reject phrases that are mostly function words or grammatical fragments
    bad_tokens = {"be", "is", "are", "was", "were", "will", "would", "could", "should", "do", "does", "did", "may", "shall", "can", "must", "have", "has", "had", "and", "or", "if", "then", "one", "two", "three", "four", "five", "them", "their", "his", "her", "its", "that", "this", "those", "these", "to", "of", "for", "with", "from", "by", "as", "at", "in", "on", "commit", "commits", "punish", "punished", "jointly", "conjointly", "joint", "they", "them", "their", "person", "persons"}
    ...
    if not content:
        return False
    # need at least one substantive token
...
def _extract_salient_phrases(text: str) -> List[str]:
    """Heuristic phrase extraction used only for chunk grounding.

    This is intentionally conservative. We prefer a few high-signal phrases over
    noisy word salad.
    """
    ...
    phrases: List[str] = []
    # Sentence fragments around punctuation / commas / semicolons.
    parts = re.split(r"[;:\n\.\(\)]", text)
    for part in parts:
        part = _normalize_space(part)
        if not part:
            continue
        # Keep short legalish fragments only.
        if 3 <= len(part.split()) <= 7:
            phrases.append(part)

    # Add a few noun-like sequences.
...
class ChunkBank:
    ...
    def _add_phrase(self, phrase: str, source: str) -> None:
        ...
        if not _is_good_concept_phrase(phrase):
            return
        if phrase in {"short title", "commencement", "application"}:
            # Keep these only if paired with a more specific label.
            pass
    ...
    def _build_phrase_index(self) -> None:
        for item in self.items:
            ...
            for phrase in item.candidate_phrases():
                # Filter out ultra-generic single tokens unless they appear frequently.
                tokens = phrase.split()
                if len(tokens) == 1 and tokens[0] in GENERIC_CONCEPT_STOPWORDS:
                    continue
                self._add_phrase(phrase, source)

    def _score_phrase(self, query_tokens: Sequence[str], query_bigrams: Sequence[str], query_trigrams: Sequence[str], phrase: str) -> float:
        ...
        # Boost legal concept phrases and section title phrases a bit.
    ...
    def candidate_concepts(self, query: str, limit: int = 25) -> List[ChunkCandidate]:
        ...
        for phrase, freq in self._phrase_frequency.items():
            ...
            if score <= 0:
                continue
            # Frequency prior: common phrases are more likely to exist, but not too much.
    ...
    def candidate_sections(self, query: str, limit: int = 10) -> List[ChunkCandidate]:
        ...
        for item in self.items:
            ...
            src = item.chunk_id
            scored.append(ChunkCandidate(label=title, score=score, evidence=evidence, source=src))

        scored.sort(key=lambda x: (-x.score, len(x.label), x.label))
        return _dedupe_candidates(scored[:limit])

    def grounding_block(self, query: str, limit_concepts: int = 24, limit_sections: int = 8) -> str:
...
class QueryAnalyzer:
    ...
    def enforce_no_hallucinated_sections(self, result: Dict[str, Any]) -> Dict[str, Any]:
        query = result.get("query", "").lower()

        # Check if user explicitly mentioned section
        ...
        for t in result.get("targets", []):
            ...
            # If section not explicitly in query → remove it
            if t_type == "section" and not has_explicit_section:
                # Try to extract meaningful concept from it
                # Example: "Section 309: Robbery" → "robbery"
            ...
            if t_type == "section" and has_explicit_section:
                # ensure it's actually mentioned
    ...
    def analyze(self, query: str, allow_decomposition: bool = True) -> Dict[str, Any]:
        ...
        result = self.enforce_no_hallucinated_sections(result)
    ...
    def _analyze_single_query(self, query: str) -> Dict[str, Any]:
        ...
        # If rules are clearly confident or no LLM is available, keep rules.
        print(rule_result["confidence"]["rule_based"], end="  ")
        print(self.rule_high_threshold, end="  ")
        print(self.llm_client is None)
        
        if rule_result["confidence"]["rule_based"] >= self.rule_high_threshold or self.llm_client is None:
            rule_result["method"] = "rules"
            return rule_result

        # Let the LLM take over when rules are uncertain enough.
    ...
    def _attach_query_decomposition(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        analyzed_sub_queries = [self.analyze(sub_query, allow_decomposition=False) for sub_query in clean_sub_queries[: self.max_sub_queries]]
        result["decomposition"] = {
            "needed": True,
            "strategy": strategy,
            "reason": reason,
    ...
    def _should_decompose_query(self, result: Dict[str, Any]) -> Tuple[bool, str, bool]:
        ...
        connector_hits = len(re.findall(r"\b(and|also|plus|or|then|but|along with|as well as)\b", q))
        if connector_hits >= 2:
            reasons.append("multiple connectors suggest more than one legal issue")
            force_llm = True
        if connector_hits == 1 and len(query.split()) >= 14:
            reasons.append("single connector inside a long query may hide two issues")
            force_llm = True

        if bool(features.get("is_multi_hop", False)):
            reasons.append("query features mark it as multi-hop")
            force_llm = True
        if bool(features.get("requires_reasoning", False)):
            reasons.append("query needs legal reasoning")
        if primary in {"comparison", "hypothetical", "case_application", "consequence"}:
            reasons.append(f"primary intent '{primary}' often benefits from multiple retrieval queries")
        if any(s in {"reasoning", "case_application", "legal_condition", "legal_exception"} for s in secondary):
            reasons.append("secondary intent indicates conditional or multi-issue analysis")

        if re.search(r"\bif\b", q) and re.search(r"\bwhat (will|would|could|can) happen\b|\bwhat happens\b|\bthen what\b", q):
            reasons.append("conditional query with an outcome question")
            force_llm = True
        if re.search(r"\bdifference between\b|\bcompare\b|\bvs\b|\bversus\b", q):
            reasons.append("comparison query should be split into separate sides")
            force_llm = True
        if re.search(r"\bexception(s)?\b|\bpenalt(y|ies)\b|\bpunishment\b|\bcondition(s)?\b", q) and re.search(r"\bsection\s+\d+", q):
            reasons.append("section plus exception/punishment/condition query usually needs multiple provision lookups")
    ...
    def _rule_decompose_query(self, query: str, analysis: Dict[str, Any]) -> List[str]:
        ...
        chunks = re.split(r"\b(?:and also|also|plus|and|but|along with|as well as)\b", q, flags=re.IGNORECASE)
        if len(chunks) > 1:
            anchor = ""
            ...
            for chunk in chunks:
                ...
                if not cleaned:
                    continue
                if anchor and re.search(r"\b(caught|happen|happens|happened|apply|applies|punishment|penalty|liable|liable to)\b", cleaned, re.IGNORECASE):
        ...
        if bool((analysis.get("query_features", {}) or {}).get("is_multi_hop", False)):
            query_without_tail = re.split(
                r"\b(?:then|what happens|what will happen|what could happen|what would happen)\b",
                q,
                maxsplit=1,
                flags=re.IGNORECASE,
            )[0].strip(" ,?")
...
- Keep each sub-query focused on one legal issue only.
- Do not merge unrelated issues into a single sub-query.
...
""".strip()
        ...
        raw = self.llm_client.generate(model=self.llm_model, prompt=prompt)
    ...
    def _rule_based_analysis(self, query: str) -> Dict[str, Any]:
        ...
        # Query-to-concept mapping from rules.
    ...
    def _extract_semantic_concepts(self, query: str) -> List[str]:
        ...
        # Add top grounded concepts from chunk bank if they are semantically close.
        if self.chunk_bank is not None:
            ...
            for c in candidates:
                if not _is_good_concept_phrase(c.label):
                    continue
                # only keep grounded candidates if they overlap query words or are very strong.
                if c.score >= 1.2:
...
{{
  ...
  "confidence": "get the confidence score of your answer",
  "decomposition": {{
    "needed": true/false,
  ...
  "method": "llm",
  "sub_queries": ["optional query 1", "optional query 2"],
  "notes": ["optional note"]
}}
        ...
        if include_sub_queries and isinstance(sub_queries, list):
            for sq in sub_queries:
                if not isinstance(sq, dict):
                    continue
                # recursive validation without allowing endless nesting
                validated = self._validate_result_payload(dict(sq), include_sub_queries=False)
                clean_sub_queries.append(validated)
        result["sub_queries"] = clean_sub_queries

        # Sanity: if decomposition says no, drop subqueries.
        if not result["decomposition"]["needed"]:
            result["sub_queries"] = []

        # Keep overall confidence slightly conservative when decomposition exists.
...
python query_analysis/aq.py --chunks data/processed/artifacts2/chunks.json \
    --query "If five persons conjointly commit robbery and one of them commits murder, how will they be punished?" \
    --output query_analysis/result4.json \
    --stats
...
def main() -> None:
    ...
    analyzer = build_analyzer(args.chunks, model=args.model, enable_llm=True)
...
def summarize_chunks(bank: ChunkBank) -> Dict[str, Any]:
    ...
    # rough concept coverage: number of unique grounded phrases
