...
class Phase9HybridRetriever:
    ...
    def _bm25_terms_for_query(self, q8: Phase8Query) -> List[str]:
        ...
        terms.extend(self._clean_concepts_for_bm25(q8, max_terms=4))
        ...
        # Keep it sparse. The original query itself is already a strong signal.
    ...
    def build_faiss_text(self, q8: Phase8Query) -> str:
        # Short natural query for semantic retrieval.
        parts = [q8.query]
    ...
    def _rerank(self, query_text: str, hits: Dict[str, RetrievalHit], limit: Optional[int] = None) -> List[RetrievalHit]:
        if limit is None:
            limit = self.final_k
    ...
    def _merge_variant_hits(self, per_variant: Dict[str, List[RetrievalHit]]) -> Dict[str, RetrievalHit]:
        ...
        for hit in merged.values():
            ...
            if support >= 2:
                hit.score = _clamp(hit.score + min(0.08, 0.02 * support))
    ...
    def _process_single_query(self, q8: Phase8Query, variant_name: str) -> List[RetrievalHit]:
        ...
        return self._rerank(q8.query, raw_hits, limit=self.final_k)
    ...
    def retrieve_one(self, phase8_item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        global_hits = self._rerank(q8.query, dict(merged), limit=self.final_k)
...
python retrieval/rq.py --phase8 query_analysis/result_2_.json  --base-dir data/processed/artifacts2  --k 5  --output retrieval/output_2.json  --no-graph
