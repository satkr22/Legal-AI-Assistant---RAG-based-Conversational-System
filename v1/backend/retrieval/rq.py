...
# SemRank-lite defaults. SemRank is used as a third retrieval channel and as a
# conservative pre-cross-encoder concept-alignment signal.
...
class Phase9HybridRetriever:
    def __init__(
        ...
        semrank_max_query_concepts: int = 8,
    ) -> None:
        ...
        self.semrank_max_query_concepts = max(1, int(semrank_max_query_concepts))
    ...
    def _ensure_semrank_index(self) -> bool:
        ...
        phrases = [str(entry["phrase"]) for entry in self.semrank_entries]
    ...
    def _apply_semrank_pre_rerank(self, hits: Dict[str, RetrievalHit], q8: Phase8Query) -> Dict[str, RetrievalHit]:
        ...
        for chunk_id, hit in hits.items():
            indices = self.semrank_chunk_to_indices.get(chunk_id, [])
            if not indices:
                continue
            try:
            ...
            weighted_scores: List[Tuple[float, int, float]] = []
            for local_pos, idx in enumerate(indices):
                sim = float(sim_matrix[:, local_pos].max())
                weight = float(self.semrank_entries[idx].get("weight", 1.0))
                weighted_scores.append((_clamp(sim * weight), idx, sim))
            if not weighted_scores:
                continue

            best_score, best_idx, best_sim = max(weighted_scores, key=lambda item: item[0])
            if best_sim < self.semrank_min_sim:
                continue

            old_score = hit.score
            hit.score = _clamp((1.0 - self.semrank_weight) * hit.score + self.semrank_weight * best_score)
            hit.source_scores["semrank_pre_rerank"] = max(
                hit.source_scores.get("semrank_pre_rerank", 0.0),
                round(best_score, 6),
            )
            hit.reasons.append("semrank_pre_rerank")
            hit.reasons.append("semrank_concept_match")

            entry = self.semrank_entries[best_idx]
            current = dict(hit.semrank or {})
            current.update(
                {
                    "enabled": True,
                    "query_concepts": query_phrases,
                    "pre_rerank_score": round(best_score, 4),
                    "best_similarity": round(best_sim, 4),
                    "best_concept": entry["phrase"],
                    "best_concept_source": entry.get("source", "concept"),
    ...
    def _retrieve_for_variant(self, variant: QueryVariant) -> Dict[str, RetrievalHit]:
        ...
        def add_hit(
            ...
            if semrank_diag:
                current = dict(hit.semrank or {})
                current.update({"enabled": True, "source": "retrieval", **semrank_diag})
                hit.semrank = current
    ...
    def _graph_expand(self, hits: Dict[str, RetrievalHit], q8: Phase8Query) -> Dict[str, RetrievalHit]:
        ...
        for seed in seeds:
            ...
            for nid in neighbors:
                ...
                # Apply the same domain-agnostic adjustments as _retrieve_for_variant
                # so graph-expanded chunks are held to the same relevance standard.
                overlap = _compute_concept_overlap(query_concepts, rec.legal_concepts)
                if overlap > 0:
    ...
    def retrieve_one(self, phase8_item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        results_with_global_rerank = self._format_results(global_hits, q8=q8)
        ...
        return {
            ...
            "retrieval": {
                ...
                "per_query_results": {
                    name: self._format_results(hits, q8=q8)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--no-semrank", action="store_true", help="Disable SemRank concept retrieval and pre-reranking")
    p.add_argument("--semrank-top-k", type=int, default=20, help="SemRank candidate count per variant")
    p.add_argument("--semrank-weight", type=float, default=0.18, help="SemRank pre-rerank blend weight")
    p.add_argument("--semrank-min-sim", type=float, default=0.62, help="Minimum concept similarity for SemRank matches")
    return p
